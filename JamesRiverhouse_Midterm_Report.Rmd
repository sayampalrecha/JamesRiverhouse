...
# Calculate percentage and cumulative percentage for Origin-Destination combinations
origin_dest_filtered <- ds %>%
  group_by(Origin, Destination) %>%
  summarise(Count = n(), .groups = "drop") %>%
  arrange(desc(Count)) %>%
  mutate(Percentage = (Count / sum(Count)) * 100,
         Cumulative_Percentage = cumsum(Count / sum(Count)) * 100) %>%
  filter(Cumulative_Percentage <= 80)  # Keep rows within 80% of cumulative frequency

# View the filtered data
print(origin_dest_filtered)

# Join back with the original dataset (if needed)
ds_filtered <- ds %>%
  semi_join(origin_dest_filtered, by = c("Origin", "Destination"))
...
Let's check accuracy for the model built using the features from the best model above:

```{r}
# For model2
# Predicted values
predicted_model2 <- predict(model2, newdata = ds_filtered)

#MSE for model2
mse_model2 <- mean((ds_filtered$Arrival_Delay_min - predicted_model2)^2)

# RMSE for model2
rmse_model2 <- sqrt(mse_model2)

cat("Model 2 - Mean Squared Error (MSE):", mse_model2, "\n")
cat("Model 2 - Root Mean Squared Error (RMSE):", rmse_model2, "\n")

predicted_model3 <- predict(model3, newdata = ds_filtered)

mse_model3 <- mean((ds_filtered$Arrival_Delay_min - predicted_model3)^2)

rmse_model3 <- sqrt(mse_model3)

cat("Model 3 - Mean Squared Error (MSE):", mse_model3, "\n")
cat("Model 3 - Root Mean Squared Error (RMSE):", rmse_model3, "\n")
```
The RMSE of around 9.25-9.76 minutes suggests that, on average, the model's predictions deviate by approximately 9-10 minutes from the actual arrival delay, Given more features in the dataset, this error can be reduced to as minimum as possible.
...
By analyzing flight delay data through exploratory data analysis (EDA) techniques in R,
we can uncover valuable insights into the causes and patterns of delays across U.S.
airlines and airports. Identifying trends among the most delayed airlines helps allocate
resources more effectively to reduce operational inefficiencies. Similarly, understanding
how delay patterns vary across different times of day and seasons allows both airlines
and airports to make informed decisions about scheduling and staffing. Ultimately, this
analysis provides actionable insights that can help mitigate flight delays and improve
overall passenger satisfaction while enhancing operational resilience within the aviation
industry.
